...
import ComponentEllipsis from './Ellipsis';
import { axiosGetfetcher } from '../../../hooks/api';
import TokenContext from '../../../store/TokenContext';

interface IPropsComponentHeader {}

const ComponentHeader: React.FC<IPropsComponentHeader> = ({}) => {
  ...
  const ellipsisElementRef = React.useRef<HTMLDivElement | null>(null);
    ...
    <div className="h-[100px] font-semibold text-gray-300 font-noto header-container">
              ...
              >
                ...
                {isVisibleEllipsis && (
                  <div className="fixed z-[999] min-w-max">
                    <ComponentEllipsis logoutHandler={logoutHandler} />
                  </div>
                )}
              </div>
            )}
      ...
      <div className="flex items-center justify-between h-full mx-auto md:hidden">
        <div className="flex items-center justify-center w-full">
          <Link to="/">
            {!!dataHeaderLogo && dataHeaderLogo.length > 0 ? (
              <img src={dataHeaderLogo[0].content} width="110.24" alt="Logo" />
            ) : (
              ''
            )}
          </Link>
