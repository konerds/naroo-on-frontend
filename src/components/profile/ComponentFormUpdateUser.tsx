...
import { showError, useSWRProfileUser } from '../../hooks/api';
...
const ComponentFormUpdateUser: FC<IPropsComponentFormUpdateUser> = ({
  fieldType,
  id,
  userField,
}) => {
  const token = useRecoilValue(stateToken);
  const { mutate: mutateProfileUser } = useSWRProfileUser();
