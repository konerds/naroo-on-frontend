...
const PageDetailLecture: FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const token = useRecoilValue(stateToken);
  const [isShowImgThumbnail, setIsShowImgThumbnail] = useState<boolean>(false);
  ...
  const { mutate: mutateListLectureByUser } = useSWRListLectureByUser();
  const {
    data: dataDetailLecture,
    mutate: mutateDetailLecture,
    error: errorDetailLecture,
    isValidating: isValidatingDetailLecture,
  } = useSWRDetailLecture(id);
  const [isLoadingDetailLecture, setIsLoadingDetailLecture] = useState(false);
  useEffect(() => {
    setIsLoadingDetailLecture(
      (!!!dataDetailLecture && !!!errorDetailLecture) ||
        isValidatingDetailLecture,
    );
  }, [dataDetailLecture, isValidatingDetailLecture, errorDetailLecture]);
  const handlerPlayLecture = async () => {
    if (!!dataDetailLecture) {
      if (dataDetailLecture.status === 'accept') {
        navigate(`/lecture-play/${id}`);
      } else if (!!!dataDetailLecture.status) {
        if (!!token && !!dataInfoMe && dataInfoMe.role === 'student') {
          try {
  ...
  const handlerAddQuestion = async () => {
    try {
  ...
  return (
    <div
      className={`w-full max-w-[100vw]${
        isLoadingDetailLecture && ' mt-[-3px]'
      }`}
    >
      {isLoadingDetailLecture ? (
        <ComponentSkeletonCustom className="w-full-important min-h-screen" />
      ) : (
        <>
          {!!dataDetailLecture && !!!errorDetailLecture && (
            <>
              {isShowImgThumbnail && !!dataDetailLecture.thumbnail && (
